...
import { featuredEvents, eventServicesList } from '../data/services'
import { contact } from '../data/contact'

export default function EventManagement() {
  return (
    <section className="py-20 sm:py-28 bg-ivory-soft">
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        <Reveal>
          <div className="flex items-center gap-3 mb-4">
            <span className="grid place-items-center w-11 h-11 rounded-2xl bg-wine/10 text-wine">
              <PartyPopper size={20} />
            </span>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <h2 className="font-display text-3xl sm:text-4xl md:text-5xl font-semibold text-plum text-balance">
                Bagavan Event Management
              </h2>
              <p className="mt-3 max-w-xl text-ink/70 leading-relaxed">
                Creative event planning and celebration management, working hand in
                hand with Kallai Gift Center to bring every occasion to life.
              </p>
            </div>
            <a
              ...
              className="inline-flex items-center gap-2 self-start md:self-auto px-6 py-3 rounded-full bg-wine text-ivory font-semibold hover:bg-plum transition-colors shadow-card"
            >
              Plan Your Event
              <ArrowRight size={17} />
            </a>
        ...
        <div className="mt-12 grid lg:grid-cols-12 gap-8 items-center">
          {/* Event Services Listed by Words (Left side for alternating visual rhythm) */}
          <div className="lg:col-span-7 order-2 lg:order-1">
            <Reveal>
              <div className="mb-6">
                ...
                <h3 className="font-display text-2xl sm:text-3xl font-semibold text-plum">
                  Event Planning &amp; Stage Decoration
                </h3>
              </div>
            </Reveal>

            <div className="grid sm:grid-cols-2 gap-4">
              {eventServicesList.map((item, i) => (
                <Reveal key={item.id} delay={(i % 4) * 60}>
                  <div className="h-full p-4 sm:p-5 rounded-2xl bg-white border border-wine/10 shadow-sm hover:shadow-card hover:border-wine/40 transition-all duration-300">
            ...
            <Reveal className="mt-8 flex flex-wrap items-center gap-4">
              ...
              <a
                ...
                className="px-7 py-3 rounded-full border border-wine/25 text-wine font-semibold hover:bg-wine/5 transition-colors text-sm"
              >
          ...
          <div className="lg:col-span-5 order-1 lg:order-2">
            <Reveal>
              <div className="group relative rounded-4xl overflow-hidden shadow-soft aspect-[4/5] bg-plum-dark">
                ...
                <span className="absolute top-5 left-5 px-3 py-1.5 rounded-full text-xs font-semibold bg-peach text-plum uppercase tracking-wider shadow">
                  {featuredEvents.badge}
                </span>
